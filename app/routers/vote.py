...
def vote(vote: schemas.Vote, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    posts = db.query(model.Post).filter(model.Post.id == vote.post_id).first()
    if not posts:
    ...
    if vote.dir == 1: # wants to add new vote or let's say likes
        if found_vote:
        ...
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                detail=f"User {current_user.id} has not casted vote for the post {vote.post_id} YET!")
